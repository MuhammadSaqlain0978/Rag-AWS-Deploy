...
class UniversityRAGBot:
    ...
    def answer_query(self, user_question, chat_history=None):
        """Answer university-related queries using RAG with enhanced context and chat history"""
        ...
        try:
            ...
            # Step 4: Create enhanced prompt with chat history
            history_context = ""
            if chat_history and len(chat_history) > 0:
                # Include last 2-3 conversation turns for context
                recent_history = chat_history[-4:]  # Last 2 user-assistant pairs
                history_context = "\n\nPREVIOUS CONVERSATION (for context only):\n"
                for msg in recent_history:
                    if msg['role'] == 'user':
                        history_context += f"Student: {msg['message']}\n"
                    else:
                        history_context += f"Assistant: {msg['message']}\n"
                history_context += "\n" + "="*50 + "\n"
    ...
    - Answer in bullets if the response requires to be in bullets
    - Provide the full information
    - Use the previous conversation as context, but focus on answering the current question
    {history_context}
    ...
    University Assistant Response:"""
            ...
            # Step 6: Get response from Mistral
            ...
            # Step 7: Collect source information
            sources_info = []
...
#             relevant_docs = self.vector_store.similarity_search(
#                 user_question, 
#                 k=4  # Get top 6 most relevant chunks
#             )
